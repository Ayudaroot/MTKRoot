...
Enumeration CustomEvents
  ...
  #PB_EventType_Enable
  #PB_EventType_Unauthorized
  #PB_EventType_DeviceNotFound
  #EventType_RecordingEnd
  #EventType_VideoFinished
EndEnumeration
...
Global VideoThread.i = 0       ; Para controlar el hilo
Global RecordingActive.i = #False ; Para saber si está grabando
Global VideoFinishedFilename.s = ""
Global RecordingStartTimer.i = 0
Global RecordingSeconds.i = 0

IncludeFile("proceduresv32.pb")
...
DataSection
  ...
  Image28:
  IncludeBinary "logos\upload.png"
    Image29:
...
; --- Ventana(510x680) ---
If OpenWindow(#Window_0, 0, 0, 510, 680, "MTKroot v3.2 (2025)", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_MinimizeGadget)
  version.s = "3.2"
  
If CreateImageMenu(0, WindowID(#Window_0), #PB_Menu_ModernLook)
    MenuTitle("File")
    MenuItem(50, "Load", Images(1))
EndIf
  
;-===== LEFT SECTION (Main actions) =====
FrameGadget(100, 10, 10, 200, 280, "Root Tools")
ButtonGadget(0, 20, 40, 180, 30, "📁 Process Patch")
GadgetToolTip(0, "Transfers the boot file for patching (Required for root)")
ButtonGadget(16, 20, 80, 180, 30, "📱 Root Apps") 
...
ButtonGadget(8, 20, 160, 180, 30, "🔓 Unlock BL")
...
OptionGadget(20, 230, 190, 90, 20, "🌱 Root") 
OptionGadget(21, 230, 220, 90, 20, "💀 Unroot")

;-===== RIGHT SECTION (ADB/Fastboot) =====
FrameGadget(102, 360, 10, 140, 250, "Connection")
ButtonGadget(3, 370, 40, 120, 30, "📡 Test ADB")
GadgetToolTip(3, "Verifies ADB connection ⚠️ Ensure USB debugging is ON") 
ButtonGadget(4, 370, 80, 120, 30, "⚡ Test Fastboot") 
GadgetToolTip(4, "Verifies Fastboot connection") 
ButtonGadget(5, 370, 120, 120, 30, "🔌 Enter Fastboot")
GadgetToolTip(5, "Reboots into Fastboot mode") 
ButtonGadget(6, 370, 160, 120, 30, "🔋 Exit Fastboot")
GadgetToolTip(6, "Reboots into normal mode") 
ButtonGadget(7, 370, 200, 120, 30, "🔄 Reboot")

; ===== BOTTOM SECTION (Console/Extra buttons) =====
EditorGadget(2, 10, 300, 490, 250, #PB_Editor_ReadOnly|#PB_Editor_WordWrap)
SetGadgetColor(2, #PB_Gadget_BackColor, RGB(0, 0, 0))
SetGadgetColor(2, #PB_Gadget_FrontColor, RGB(126, 178, 25))
SetGadgetFont(2, LoadFont(2, "Consolas", 12)) ; Fuente monoespaciada
AddGadgetItem(2, -1, ">>> MTKroot v3.2 - Ready <<<") ; Línea inicial descriptiva

; --- Bottom buttons ---
FrameGadget(103, 265, 550, 130, 95, "Actions") 
ButtonGadget(151, 10, 570, 120, 30, "📺 Screen Control")
GadgetToolTip(151, "Remote device control") 
ButtonGadget(152, 10, 608, 120, 30, "📸 Screenshot") 
GadgetToolTip(152, "Take a Screenshot connected device") 
ButtonGadget(153, 140, 608, 120, 30, "🎥 Record Video")
GadgetToolTip(153, "Start recording Video") 
ButtonGadget(1, 270, 570, 120, 30, "⚡ Root/Unroot")
ButtonGadget(18, 270, 608, 120, 30, "🔓 BL Status")
GadgetToolTip(18, "Verifies bootloader status") 
GadgetToolTip(1, "Executes selected action") 
ButtonImageGadget(131, 400, 560, 50, 45, Images(27))  ; Console icon
GadgetToolTip(131, "Open ADB console") 
ButtonImageGadget(250, 450, 560, 50, 45, Images(28))  ; Update icon
GadgetToolTip(250, "Send File") 
ButtonGadget(17, 140, 570, 120, 30, "💾 Extract boot")
GadgetToolTip(17, "Get boot.img Partition of rooted device")
...
While (iLine < iTotal)
       ...
       If Not IsThread(ThreadID)
         DisableGadget(151, 1)
         ThreadID = CreateThread(@ScreenViewerThread(), 0)
       EndIf
       DisableGadget(151, 0)
       AutoScrollLog(2)
...
Case 1;--Root/Unroot-----------------------   
  ...
  If GetGadgetState(20) <> 0
    ...
    ; Usamos la función mejorada
    bootpart.s = DetermineBootPartition()
    ...
    If resroot = #PB_MessageRequester_Yes
      o$ = adb("fastboot boot "+Chr(34)+rai$+"root\"+"root.img"+Chr(34))
    Else
  ...
  ElseIf GetGadgetState(21) <> 0
    ...
    ; Usamos la función mejorada también para unroot
    bootpart.s = DetermineBootPartition()
    o$ = adb("fastboot flash "+bootpart+" "+Chr(34)+rai$+"unroot\"+"unroot.img"+Chr(34))
...
Else
  SetGadgetText(2,"")
  AddGadgetItem(2, -1, "❌ Device disconnected - Check cable and USB debugging")
  abra3()
EndIf
DisableGadget(3,0)
...
Case 5 ;--ENTER FASTBOOT
 ...
 AddGadgetItem(2, -1, o$)
 DisableGadget(5,0)
SetGadgetState(20, #False)
SetGadgetState(21, #False)
...
Case 131;--ADB CONSOLE
  DisableGadget(131,1)
  RunProgram("cmd")
  DisableGadget(131,0)
Case 250;-- SEND FILE
   file$ = OpenFileRequester("Selecciona archivo", "", "Todos los archivos|*.*", 0)
  If file$ <> ""
     o$ = adb("adb push " + Chr(34) + file$ + Chr(34) + " /sdcard/")
     AddGadgetItem(2, -1, o$)
     AddGadgetItem(2, -1, GetFilePart(file$)+" transfered to /sdcard/")
    AutoScrollLog(2)
  EndIf
...
AutoScrollLog(2)
   ...
   AutoScrollLog(2)
     ...
     If Len(o$)>29
       ...
       o$=adb("adb exec-out screencap -p > ../screenshot"+Str(Random(9999))+".png"):AddGadgetItem(2, -1, o$)
       AddGadgetItem(2, -1, ">Screenshot"+Str(Random(9999))+".png saved.")
     Else
       abra2()
     EndIf
     DisableGadget(152,0)
...
Case 153 ;-- Record Video
    Select EventType()
        Case #PB_EventType_FirstCustomValue ; Actualización de tiempo
            If RecordingActive
                SetGadgetText(153, "⏹ " + Str(60 - RecordingSeconds) + "s")
                ; Actualizar consola cada 5 segundos
                If RecordingSeconds % 5 = 0
                    AddGadgetItem(2, -1, "⏱ Tiempo restante: " + Str(60 - RecordingSeconds) + "s")
                EndIf
            EndIf
            
        Case #PB_EventType_FirstCustomValue + 1 ; Finalización
            SetGadgetText(153, "🎥 Record Video")
            AddGadgetItem(2, -1, "✅ Video guardado: " + VideoFinishedFilename)
            RecordingActive = #False
            
        Default ; Clic manual
            If Not RecordingActive
                ; Iniciar
                RecordingActive = #True
                RecordingSeconds = 0
                SetGadgetText(153, "⏹ 60s")
                AddGadgetItem(2, -1, "⏎ Iniciando grabación...")
                VideoThread = CreateThread(@GrabarVideo(), 0)
            Else
                ; Cancelar
                RecordingActive = #False
                RunProgram("cmd", "/c adb shell pkill -l SIGINT screenrecord", "", #PB_Program_Hide)
                SetGadgetText(153, "🎥 Record Video")
                AddGadgetItem(2, -1, "⏹ Cancelado a los " + Str(RecordingSeconds) + "s")
              EndIf
              AutoScrollLog(2)
          EndSelect
          
          Case 17 ;-- Beto boot.img
BackupBootIMG()
      ...
      Repeat
        ...
        Case 121
          If adbon =1
            ...
            SetActiveWindow(#Window_0)
            AddGadgetItem(2, -1, "Waiting...")
            o$=adb("adb install -t "+Chr(34)+rai$+"app\"+"kernelsu.apk"+Chr(34))
            AddGadgetItem(2, -1, o$)
            If o$="":isap.s="OK":EndIf
            MessageRequester("","App installed",0)
            finapp = 1
        ...
        Case 122
          If adbon =1
            ...
            SetActiveWindow(#Window_0)
            AddGadgetItem(2, -1, "Waiting...")
            o$=adb("adb install -t "+Chr(34)+rai$+"app\"+"setcpu-2-0-5.apk"+Chr(34))
            AddGadgetItem(2, -1, o$)
            If o$="":isap.s="OK":EndIf
            MessageRequester("","App installed",0)
            finapp = 1
        ...
        Case 127 ;--INSTALL MAGISK
          ...
          Else
            MessageRequester("Error","No device")
          EndIf
          DisableGadget(127,#False)
        Case 128 ;--REMOVE MAGISK
          DisableGadget(128,#True)
          If adbon =1
            AddGadgetItem(2, -1, "Waiting...")
...
; Executable = MTKRoot31\MTKroot3.2.exe